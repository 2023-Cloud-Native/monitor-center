...
def test_electricity_outdated():
    ...
    assert electricity_manager.updated_time == electricity_manager.format_time(
        "2100-01-01 00:00"
    )
...
def test_electricity_normal():
    ...
    if electricity_manager.data["updated_time"] != "N/A":
        ...
        assert (
            isinstance(electricity_manager.data["north_gen"], float)
            and electricity_manager.data["north_gen"] >= 0
        )
        assert (
            isinstance(electricity_manager.data["north_use"], float)
            and electricity_manager.data["north_use"] >= 0
        )
        assert (
            isinstance(electricity_manager.data["central_gen"], float)
            and electricity_manager.data["central_gen"] >= 0
        )
        assert (
            isinstance(electricity_manager.data["central_use"], float)
            and electricity_manager.data["central_use"] >= 0
        )
        assert (
            isinstance(electricity_manager.data["south_gen"], float)
            and electricity_manager.data["south_gen"] >= 0
        )
        assert (
            isinstance(electricity_manager.data["south_use"], float)
            and electricity_manager.data["south_use"] >= 0
        )
